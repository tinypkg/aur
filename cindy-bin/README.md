...
[上游主页](https://github.com/makecindy/cindy) · [AUR](https://aur.archlinux.org/packages/cindy-bin)

此包使用上游 **国际版（global）** Linux x64 deb，初始版本为 `0.1.73`，仅支持 `x86_64`。

```sh
yay -S cindy-bin
```

安装后从应用菜单打开 Cindy，或运行 `cindy`。两个版本使用相同的命令、桌面入口和安装路径，因此 `cindy-bin` 与 `cindy-cn-bin` 互斥；切换版本时由包管理器替换。

## 自动更新

[GitHub Actions](https://github.com/tinypkg/aur/actions/workflows/update-cindy-bin.yml) 每 12 小时检查 `makecindy/cindy` 的最新 GitHub Release。发布前验证资产名称、deb 内版本和 amd64 架构，并拒绝降级。
